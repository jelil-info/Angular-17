...
const formModel = require("../formModel/form");
...
app.post("/submit", async (req, res) => {
  ...
  const secretkey = process.env.recaptcha_key;//the secret key from your google admin console;

  //token validation url is URL: https://www.google.com/recaptcha/api/siteverify
  // METHOD used is: POST

  const url = `https://www.google.com/recaptcha/api/siteverify?secret=${this.secretKey}&response=${token}&remoteip=${req.connection.remoteAddress}`
  ...
  /*fetch(url, {
    ...
    .then((google_response) => {
      ...
      if (google_response.success == true) {
        ...
        return console.log('Server received did not our data');
...
`)*/
  ...
  const internalEmail = {
      ...
      text: `A new contact form has been submitted to infotechcrush.com in localhost!
